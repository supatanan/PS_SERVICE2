...
uses
  ...
  dxSkinXmas2008Blue, Vcl.Menus, Data.DB, ppParameter, ppDesignLayer, ppModule,
  raCodMod, ppBands, ppCtrls, ppVar, ppStrtch, ppMemo, ppPrnabl, ppClass,
  ppCache, ppProd, ppReport, MemDS, DBAccess, Uni, ppDB, ppComm, ppRelatv,
  ppDBPipe, Vcl.StdCtrls, cxButtons, cxTextEdit, cxMaskEdit, cxDropDownEdit,
  cxCalendar, dxGDIPlusClasses, Vcl.ExtCtrls, RzPanel, ppBarCod;

type
  Treport_frm2_2 = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Panel2: TPanel;
...
procedure Treport_frm2_2.cxButton1Click(Sender: TObject);
begin
  Close;
end;

procedure Treport_frm2_2.cxButton2Click(Sender: TObject);
var
  path: string;
begin
...
' tborder_service.trainsaction_trype_id = 1 and  '+
...
'tborder_service.date_service BETWEEN '''+FormatDateTime('yyyy-mm-dd', ds1.Date)+'''and  '''+FormatDateTime('yyyy-mm-dd',ds2.Date)+'''   ';
  ...
  open;
   ...
   ppReport1.Template.FileName := path + '\report\report2.rtm';
...
procedure Treport_frm2_2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  report_frm2_2 := nil;
...
procedure Treport_frm2_2.FormShow(Sender: TObject);
begin
  ds1.Date := Now;
  ds2.Date := Now;
