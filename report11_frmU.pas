...
uses
  ...
  cxGrid, dxGaugeCustomScale, dxGaugeQuantitativeScale, dxGaugeCircularScale,
  dxGaugeControl, cxGridChartView, cxGridDBChartView, cxContainer, Vcl.ComCtrls,
  dxCore, cxDateUtils, Vcl.Menus, ppParameter, ppDesignLayer, ppModule,
  raCodMod, ppBands, ppCtrls, ppBarCod, ppVar, ppStrtch, ppMemo, ppPrnabl,
  ppClass, ppCache, ppProd, ppReport, ppDB, ppComm, ppRelatv, ppDBPipe,
  cxButtons, cxTextEdit, cxMaskEdit, cxDropDownEdit, cxCalendar,
  dxGDIPlusClasses, RzPanel;

type
  Treport11_frm = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Panel2: TPanel;
...
var
  report11_frm: Treport11_frm;

implementation

{$R *.dfm}

uses MyMDU, Main_frmU;

procedure Treport11_frm.cxButton1Click(Sender: TObject);
begin
  Close;
end;

procedure Treport11_frm.cxButton2Click(Sender: TObject);
var
  path: string;
begin
...
' tborder_service.tbstatus_trype_id=5  and  '+
...
open;
  ...
  ppReport1.Template.FileName := path + '\report\report11.rtm';
  ppReport1.Template.LoadFromFile;

end;

procedure Treport11_frm.FormCanResize(Sender: TObject; var NewWidth,
...
procedure Treport11_frm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     Action  := caFree;
     report11_frm := nil;
...
procedure Treport11_frm.FormShow(Sender: TObject);
begin
  ds1.Date := Now;
  ds2.Date := Now;
