...
uses
  ...
  ppDesignLayer, ppModule, raCodMod, ppBands, ppCtrls, ppBarCod, ppVar,
  ppStrtch, ppMemo, ppPrnabl, ppClass, ppCache, ppProd, ppReport, MemDS,
  DBAccess, Uni, ppComm, ppRelatv, ppDB, ppDBPipe, cxGridLevel,
  cxGridCustomTableView, cxGridTableView, cxGridDBTableView, cxClasses,
  cxGridCustomView, cxGrid, Vcl.StdCtrls, cxButtons, cxTextEdit, cxMaskEdit,
  cxDropDownEdit, cxCalendar, dxGDIPlusClasses, Vcl.ExtCtrls, RzPanel, RzLabel;

type
  TSystem_report_all_f = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Panel2: TPanel;
...
var
  System_report_all_f: TSystem_report_all_f;

implementation

{$R *.dfm}

procedure TSystem_report_all_f.cxButton1Click(Sender: TObject);
begin
   Close;
end;

procedure TSystem_report_all_f.cxButton2Click(Sender: TObject);
var
  path: string;
begin
...
'tborder_service.date_service BETWEEN '''+FormatDateTime('yyyy-mm-dd',ds1.Date)+'''and  '''+FormatDateTime('yyyy-mm-dd',ds2.Date)+'''   ';
  ...
  ppReport1.Template.FileName := path + '\report\System_all_report.rtm';
  ppReport1.Template.LoadFromFile;

end;

procedure TSystem_report_all_f.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   Action := caFree;
   System_report_all_f := nil;
end;

procedure TSystem_report_all_f.FormShow(Sender: TObject);
begin
 ds1.Date := Now;
  ds2.Date := Now;
  qry_companey.Active := True;
