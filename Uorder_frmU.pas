...
uses
  ...
  cxDataStorage, cxNavigator, Data.DB, cxDBData, cxImageComboBox, cxGridLevel,
  cxGridCustomTableView, cxGridTableView, cxGridDBTableView, cxClasses,
  cxGridCustomView, cxGrid, Vcl.StdCtrls, cxButtons, cxTextEdit, cxMaskEdit,
  cxDropDownEdit, cxCalendar, RzLabel, Vcl.ExtCtrls, cxPC, Vcl.ImgList, MemDS,
  DBAccess, Uni, RzPanel, cxCheckBox;

type
  TUorder_frm = class(TForm)
...
procedure TUorder_frm.cxButton1Click(Sender: TObject);
begin
          with qry_select do
          begin
            Close;
...
'WHERE     tborder_service.tbconfirm_order_id4=0                            '+
...
procedure TUorder_frm.cxButton2Click(Sender: TObject);
begin
 Close;
end;

procedure TUorder_frm.cxButton3Click(Sender: TObject);
begin
       ExportGridToExcel('Export.xls',cxGrid1, True, True, True);
  ShellExecute(Handle, 'open', PChar('Export.xls'), nil, nil, SW_NORMAL);
end;

procedure TUorder_frm.chkClick(Sender: TObject);
begin
...
procedure TUorder_frm.cxGrid1DBTableView1Column2GetDisplayText(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AText: string);
begin
...
procedure TUorder_frm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
      Action := cafree;
     Uorder_frm := nil;
end;

 procedure TUorder_frm.FormShow(Sender: TObject);
begin
 ds1.Date := Now;
  ds2.Date := Now;
